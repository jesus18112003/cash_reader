...
import SumManager from "./SumManager"; // Importar el nuevo componente
...
const Camera = () => {
    ...
    // Usar el componente SumManager
    const { total, sumMode, toggleSumMode, narrateBillAndTotal, resetTotal } = SumManager({ toggleModel });

    // Modificar useVoiceInterface para incluir el comando "suma"
    const {
        error: voiceError,
        start,
        stop,
        ...
        callToggleSumMode: toggleSumMode, // Nuevo comando
        debug: true,
    });

    // Modificar sendPhotoToAPI para narrar el valor del billete y el total acumulado
    const sendPhotoToAPI = useCallback(
        async (imageData) => {
            try {
                ...
                // Narrar el valor del billete y el total acumulado
                if (response.data.detections && response.data.detections.length > 0) {
                    const label = response.data.detections[0].label;
                    const narrationMessage = narrateBillAndTotal(label);
                    if (narrationMessage) {
                        setNarration(narrationMessage); // Narrar el valor del billete y el total acumulado
                    }
                }
            } catch (error) {
        ...
        [processResponse, toggleModel, narrateBillAndTotal]
    );
